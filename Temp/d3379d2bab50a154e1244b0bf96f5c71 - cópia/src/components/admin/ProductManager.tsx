...
const ProductManager: React.FC = () => {
    ...
    const uploadFileForProduct = async (file: File, id: string) => {
        const form = new FormData();
        form.append('file', file);
        form.append('dir', 'products');
        try {
            const res = await fetch('/api/upload.php', { method: 'POST', body: form });
            if (!res.ok) throw new Error('Upload failed');
            const data = await res.json();
            handleInlineChange(id, 'imageUrl', data.url);
        } catch (err) {
    ...
    const handleImageFile = (file: File | null, id: string) => {
        if (file) uploadFileForProduct(file, id);
    };
    ...
    const handleImagePaste = (e: React.ClipboardEvent, id: string) => {
        ...
        if (item) uploadFileForProduct(item.getAsFile() as File, id);
    };
        ...
        <div>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingProduct ? 'Editar Produto' : 'Adicionar Produto'}>
                 <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div><label className="block text-sm font-medium mb-2">Imagem</label><ImageUploader onImageUpload={(url) => setFormData(p=>({...p, imageUrl: url}))} uploadDir="products" />{formData.imageUrl && <img src={formData.imageUrl} alt="Preview" className="mt-4 max-h-40 mx-auto rounded" />}</div>
